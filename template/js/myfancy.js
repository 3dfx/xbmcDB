...
/*
	$("#fancybox-overlay").bind('click', function() {
		alert('lala');
		$('#xbmcDB').css('cursor', 'auto');
	});

	$(".fancy_iframe").bind('click', function() {
		$('#xbmcDB').css('cursor', 'wait');
	}).fancybox({
*/
	...
	$("a.hoverpic").hover(function(e){
		...
		$("#hoverpic")
			.css("top",(e.pageY - correctYoffset(e)) + "px")
	...
	function correctYoffset(e) {
		var CWOffset = $(window).height() - e.pageY;
		var res = yHoverPicOffset;
		
		//console.log( e.pageY + ', ' + CWOffset + ', ' + yHoverPicOffset );
		if (e.pageY < 150) { res = 0; }
		else if (CWOffset > yHoverPicOffset) { res = 90; }
		//console.log( 'yHoverPicOffset: ' + yHoverPicOffset );
