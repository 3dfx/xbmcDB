...
include_once "xbmcJson.php";

if (!isAdmin()) { return; }
function getSymlinkSrc() { return '/hdd/'; }
...
//
$_CONFIG['main_title'] = "MP3 Explorer";
...
//
$_CONFIG['hidden_files'] = array(".ftpquota", ".php", "index.php~", ".htaccess", ".htpasswd", ".jpg", ".png", ".avi", ".mkv", ".flv", ".sh", ".py", ".cue", ".cue_");

$_CONFIG['whitelist_files'] = array(".mp3", ".m4a", ".flac");
...
//
$_CONFIG['starting_dir'] = "./fExplorer/";
...
class Dir
{
	...
	{
		$special = htmlspecialchars($this->name);
		return empty($special) ? $this->name : $special;
	}
...
class Location
{
	...
	// 
	function getDir($prefix, $encoded, $html, $up, $stripStart = false)
	{
		$dir = "";
		if($prefix == true)
			$dir .= "./";
		...
		return !$stripStart ? $dir : str_replace(str_replace('./', '', EncodeExplorer::getConfig('starting_dir')), '', $dir);
		#return $dir;
	}
...
class EncodeExplorer
{
	...
	function readDir()
	{
		...
		//
		#echo $this->location->getFullPath();
		if($open_dir = @opendir($this->location->getFullPath()))
		{
			...
			while ($object = readdir($open_dir))
			{
				if($object != "." && $object != "..") 
				{
					...
					} else if(in_array('.'.$ext, EncodeExplorer::getConfig('whitelist_files'))) {
	...
	function formatModTime($time)
	{
		...
		return empty($timeformat) ? '' : date($timeformat, $time);
	}
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#myNavbar').load( './navbar.php?maself=0', function() { /*if (isAdmin) { initNavbarFancies(); }*/ } );
	});
</script>
</head>
...
<tr class="row one header">
	...
	<td style="display:none;" class="changed"><?php $timeformat = EncodeExplorer::getConfig("time_format"); if (!empty($timeformat)) { print $this->makeArrow("mod"); } ?></td>
...
if($this->dirs)
{
	foreach ($this->dirs as $dir)
	{
		...
		#$pItem = "playItem('".getSymlinkSrc().$this->location->getDir(false, true, false, 0).$dir->getNameEncoded()."/')";
		...
		print "<a href=\"".$this->makeLink(false, false, null, null, null, $this->location->getDir(false, true, false, 0).$dir->getNameEncoded())."\" class=\"item dir\" style=\"text-decoration:none;\">";
...
if($this->files)
{
	...
	foreach ($this->files as $file)
	{
		...
		$pItem = "playItem('".getSymlinkSrc().'/'.$this->location->getDir(false, true, false, 0, true).$file->getNameEncoded()."')";
		...
		print "\t\t<a style=\"cursor:pointer; text-decoration:none;\" onclick=\"".$pItem."\"";
		...
		if($this->mobile != true)
		{
			...
			print "<td style=\"display:none;\" class=\"changed\">".$this->formatModTime($file->getModTime())."</td>\n";
		}
		if($this->mobile == false && GateKeeper::isDeleteAllowed()){
			print "<td class=\"del\">
